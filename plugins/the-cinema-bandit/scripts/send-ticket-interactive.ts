...
/**
 * Interactive Cinema Bandit ticket sender
 *
 * Prompts user for ticket information, calculates pricing,
 * generates email HTML, and sends via Gmail.
 */
...
async function main(): Promise<void> {
	console.log("🎬 Cinema Bandit - Send Ticket Email\n");
	...
	// Get booking details
	const recipientEmail = await prompt("Send to email address:");
	...
	try {
		const messageId = await sendTicketEmail(recipientEmail, movieTitle, html);
