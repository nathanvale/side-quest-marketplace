...
describe("errors", () => {
	describe("KitErrorType", () => {
		test("has all expected error types", () => {
			...
			expect(String(KitErrorType.SemanticNotAvailable)).toBe(
				"SemanticNotAvailable",
			);
	...
	describe("SEMANTIC_INSTALL_HINT", () => {
		...
		test("includes uv install command", () => {
			expect(SEMANTIC_INSTALL_HINT).toContain(
	...
	describe("isSemanticUnavailableError", () => {
		...
		test("detects semantic search error", () => {
			expect(
				isSemanticUnavailableError("semantic search is not available"),
			).toBe(true);
		});
