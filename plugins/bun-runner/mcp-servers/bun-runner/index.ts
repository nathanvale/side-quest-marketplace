...
export function parseBunTestOutput(output: string): TestSummary {
	...
	const failed = failMatch?.[1] ? Number.parseInt(failMatch[1], 10) : failures.length;
...
export function parseBiomeOutput(stdout: string): LintSummary {
	try {
		...
		return {
			error_count: summary.errorCount || diagnostics.filter((d) => d.severity === "error").length,
			warning_count:
				summary.warnCount || diagnostics.filter((d) => d.severity === "warning").length,
			diagnostics,
		};
	} catch (_e) {
		return {
 ...
 */
async function runBiomeFix(path = "."): Promise<{ fixed: number; remaining: LintSummary }> {
	// First run with --write to fix issues
	const fixProc = spawn({
		cmd: ["bunx", "@biomejs/biome", "check", "--write", "--reporter=json", path],
		stdout: "pipe",
		stderr: "pipe",
	});
 ...
 */
async function runBiomeFormatCheck(path = "."): Promise<{ formatted: boolean; files: string[] }> {
	const proc = spawn({
...
tool(
	"bun_runTests",
	{
		...
		inputSchema: {
			pattern: z
				.string()
				.optional()
				.describe("File pattern or test name to filter tests (e.g., 'auth' or 'login.test.ts')"),
		},
	},
	async (args: { pattern?: string }) => {
		...
		if (summary.failed === 0) {
			return {
				content: [{ type: "text" as const, text: `✅ All ${summary.passed} tests passed.` }],
			};
		}
...
tool(
	...
	async (args: { file?: string }) => {
		...
		if (summary.failed === 0) {
			return {
				content: [
					{ type: "text" as const, text: `✅ All ${summary.passed} tests passed in ${file}` },
				],
			};
		}
...
tool(
	"bun_lintCheck",
	{
		...
		inputSchema: {
			path: z
				.string()
				.optional()
				.describe("Path to file or directory to check (default: current directory)"),
		},
	},
	async (args: { path?: string }) => {
		...
		if (summary.error_count === 0 && summary.warning_count === 0) {
			return {
				content: [{ type: "text" as const, text: "✅ No linting issues found." }],
			};
		}
	...
	"bun_lintFix",
	{
		...
		inputSchema: {
			path: z
				.string()
				.optional()
				.describe("Path to file or directory to fix (default: current directory)"),
		},
	},
	async (args: { path?: string }) => {
...
tool(
	...
	async (args: { path?: string }) => {
		...
		if (formatted) {
			return {
				content: [{ type: "text" as const, text: "✅ All files are properly formatted." }],
			};
		}
