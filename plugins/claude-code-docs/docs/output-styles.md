...
These changes apply to the [local project level](settings.md) and are saved in `.claude/settings.local.json`. You can also directly edit the `outputStyle` field in a settings file at a different level.
...
[Agent Skills](skills.md)[Hooks](hooks-guide.md)
