...
describe("IndexGenerator", () => {
	...
	describe("formatTitle", () => {
		...
		test("formats filename with underscores", () => {
			expect(generator.formatTitle("sdk_migration-guide.md")).toBe(
				"Sdk Migration Guide",
			);
		});
		...
		test("formats multi-word with underscores and hyphens", () => {
			expect(generator.formatTitle("api_v1_getting-started.md")).toBe(
	...
	describe("createIndex", () => {
		...
		test("handles nested path filenames", async () => {
			...
			expect(content).toContain(
				"- [Sdk Migration Guide](sdk_migration-guide.md)",
			);
		});
