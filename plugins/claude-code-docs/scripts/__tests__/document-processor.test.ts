...
describe('DocumentProcessor', () => {
  ...
  describe('isValidMarkdown', () => {
    ...
    test('returns false for plain text without markdown indicators', () => {
      const content = 'This is just plain text without any markdown formatting. '.repeat(10)
      expect(processor.isValidMarkdown(content)).toBe(false)
    })
  })
  ...
  describe('saveDocument', () => {
    ...
    test('throws error for path traversal attempts', async () => {
      ...
      await expect(processor.saveDocument(url, content, filename)).rejects.toThrow(
        /path traversal/i,
      )
    })
  ...
  describe('convertHtmlToMarkdown', () => {
    test('converts HTML to Markdown', () => {
      const html = '<html><body><h1>Test Title</h1><p>Some paragraph text.</p></body></html>'
