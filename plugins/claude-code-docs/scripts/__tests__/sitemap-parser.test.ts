...
describe('SitemapParser', () => {
  ...
  describe('parseSitemap', () => {
    test('parses sitemap and filters English URLs', async () => {
      const sitemapXml = await readFile(join(fixturesDir, 'sample-sitemap.xml'), 'utf-8')
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  describe('filterEnglishUrls', () => {
    ...
    test('returns empty array when no English URLs', () => {
      ...
      const urls = ['https://code.claude.com/docs/fr/overview', 'https://code.claude.com/about']
