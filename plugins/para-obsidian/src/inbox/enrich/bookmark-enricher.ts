...
export async function enrichBookmarkWithFirecrawl(
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		...
		try {
			...
			pageContent = successResult.data?.markdown || null;

			if (!pageContent || pageContent.trim().length < 50) {
				if (log) {
					log.warn`Firecrawl returned no usable content url=${url} contentLength=${pageContent?.length ?? 0}`;
				}
				throw new BookmarkEnrichmentError(
			...
			if (log) {
				log.debug`Firecrawl success url=${url} contentLength=${pageContent.length}`;
			}
	...
	if (!pageContent) {
		throw new BookmarkEnrichmentError(
			lastError?.message || "Failed to fetch page content after retries",
			"FIRECRAWL_ERROR",
			url,
			true,
		);
	...
	// Call LLM to improve title and summary
	...
	try {
		...
		const prompt = buildImprovementPrompt(pageContent, originalTitle, url);
		...
		improvedTitle = parsed.title || originalTitle;
		// Use clean fallback summary if LLM parsing failed
		summary = parsed.summary || extractFallbackSummary(pageContent);

		if (log) {
			const usedFallback = !parsed.title || !parsed.summary;
			log.debug`LLM response parsed title="${improvedTitle.slice(0, 40)}..." summaryLength=${summary.length} duration=${llmDuration}s usedFallback=${usedFallback}`;
		}
	} catch (error) {
