...
describe('packageJsonTemplate', () => {
	...
	test('includes standard scripts', () => {
		...
		// Biome scripts are stubs since biome should be run from workspace root
		expect(parsed.scripts.format).toContain('from root')
		expect(parsed.scripts.lint).toContain('from root')
		expect(parsed.scripts.check).toContain('from root')
	})
