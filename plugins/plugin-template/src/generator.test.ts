...
describe('generatePluginStructure', () => {
  ...
  test('creates .claude-plugin/plugin.json', async () => {
    await generatePluginStructure(baseConfig, TEST_DIR)
    const pluginJsonPath = join(TEST_DIR, 'test-plugin', '.claude-plugin', 'plugin.json')
  ...
  test('creates src/ directory for typescript type', async () => {
    await generatePluginStructure(baseConfig, TEST_DIR)
    expect(existsSync(join(TEST_DIR, 'test-plugin', 'src', 'index.ts'))).toBe(true)
    expect(existsSync(join(TEST_DIR, 'test-plugin', 'src', 'index.test.ts'))).toBe(true)
  })
...
describe('generateMcpServer', () => {
  test('creates mcp-servers/ directory structure', async () => {
    ...
    expect(existsSync(join(pluginPath, 'mcp-servers', 'test-plugin'))).toBe(true)
  })
  ...
  test('creates index.ts with mcpez boilerplate', async () => {
    ...
    await generateMcpServer(pluginPath, createTemplateContext(baseConfig))
    const indexPath = join(pluginPath, 'mcp-servers', 'test-plugin', 'index.ts')
  ...
  test('creates package.json for MCP server', async () => {
    ...
    await generateMcpServer(pluginPath, createTemplateContext(baseConfig))
    const pkgPath = join(pluginPath, 'mcp-servers', 'test-plugin', 'package.json')
    expect(existsSync(pkgPath)).toBe(true)
...
describe('full plugin generation', () => {
  test('generates typescript plugin with all components', async () => {
    ...
    // MCP Server
    expect(existsSync(join(pluginPath, 'mcp-servers', 'test-plugin', 'index.ts'))).toBe(true)
    ...
    // Skills
    expect(existsSync(join(pluginPath, 'skills', 'test-plugin', 'SKILL.md'))).toBe(true)
  })
