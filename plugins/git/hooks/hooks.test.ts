...
import type { PreCompactHookInput, SessionStartHookInput } from "@anthropic-ai/claude-agent-sdk";
...
describe("SessionStartHookInput type", () => {
	...
	test("source can be startup, resume, clear, or compact", () => {
		const sources: SessionStartHookInput["source"][] = ["startup", "resume", "clear", "compact"];
...
describe("PreCompactHookInput type", () => {
	...
	test("custom_instructions can be string or null", () => {
		...
		expect(withInstructions.custom_instructions).toBe("Focus on the auth changes");
...
describe("git-context-loader", () => {
	test("can be run with valid input", async () => {
		...
		const proc = Bun.spawn(
			["sh", "-c", `echo '${JSON.stringify(input)}' | bun run git-context-loader.ts`],
			{
				cwd: import.meta.dir,
				stdout: "pipe",
				stderr: "pipe",
...
describe("session-summary", () => {
	test("can be run with valid input", async () => {
		...
		const proc = Bun.spawn(
			["sh", "-c", `echo '${JSON.stringify(input)}' | bun run session-summary.ts`],
			{
				cwd: import.meta.dir,
				stdout: "pipe",
				stderr: "pipe",
